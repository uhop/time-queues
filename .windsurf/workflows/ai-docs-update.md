...
1. Read source files in `src/` to identify the current public API (`.d.ts` files are the best reference).
...
7. Sync `.windsurfrules`, `.cursorrules`, `.clinerules` if `AGENTS.md` critical rules or code style changed:
   - These three files should be identical copies of `AGENTS.md` (with sync comment header).
